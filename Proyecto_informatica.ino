  ...
  if (mfrc522.PICC_IsNewCardPresent()){ //sale del bucle si no hay tarjeta           
   ...
   if (Serial.available() > 0) {
      ...
      if (mensaje_entrada.equals(obst)) {
          ...
          digitalWrite(trigPin, LOW); //apagamos el sensor por si se encuentra encendido
          delayMicroseconds(2); // le damos tiempo suficiente para que se apage
  
          digitalWrite(trigPin, HIGH); // encendemos el sensor y manadmos la onda sonora
          delayMicroseconds(10); 
          digitalWrite(trigPin, LOW); // lo volvemos a apagar
          duracion= pulseIn(echoPin, HIGH); // lugar donde se recibe la onda de regreso, se elige el pin echoPin ya que es el encargado de reicibir la onda ultrasónica
